function resetGameStatus(){
    ...
    /*isGameOver=False;*/
    /*playerTurnMsgElement.style.display='block';*/

    let gameBoardIndex=0;
    for(let i=0;i<3 ;i++){
        for(let j=0;j<3 ;j++){
            gameData[i][j]=0;
            const gameBoardItemElement=gameBoardElment.children[gameBoardIndex];
...
function startNewGame(){
    if(players[0].name === '' || players[1].name ===''){
        alert('please set custom name for players')
        return;
    }
    resetGameStatus();
...
function selectGameField(event){
    ...
    //players[0].symbol=X....means helps to fetch the symbol
...
function endGame(winnerId){
    ...
    if(winnerId>0){
        ...
        playerTurnMsgElement.style.display='none';
    }
    else{
